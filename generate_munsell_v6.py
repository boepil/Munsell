...
<body>
    ...
    <script type="module">
        ...
        function updatePigmentPreview() {
            ...
            let mixList = selectedIndices.map(idx => ({ name: pigmentsList[idx].name, rgb: hexToRGB(pigmentsList[idx].hex) }));
            if (mixList.length === 1) {
                mixList.push({ name: "Titanium White (PW6)", rgb: hexToRGB("#F5F5F0") });
                mixList.push({ name: "Carbon Black (PBk6)", rgb: hexToRGB("#1C1C1C") });
            }

            const combinations = generateWeightCombinations(mixList.length);
            const achievableGamut = [];
            combinations.forEach(weights => {
                const mixedRGB = mixColors(mixList.map((p, idx) => ({ rgb: p.rgb, weight: weights[idx] })));
                const mixedLab = sRGB_to_Lab(mixedRGB[0], mixedRGB[1], mixedRGB[2]);
                achievableGamut.push({ rgb: mixedRGB, lab: mixedLab });
            });
