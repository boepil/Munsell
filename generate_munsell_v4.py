...
<head>
    ...
    <title>Munsell Color Solid with Pigments (v4)</title>
    <style>
        body {
            margin: 0;
            overflow: hidden;
...
<body>
    ...
    <script type="module">
        ...
        const pigmentSpheres = [];

        // Build Pigments
        const markerGeometry = new THREE.SphereGeometry(1.0, 32, 32);
        pigmentsList.forEach(pigment => {
            ...
            // Offset sprite slightly above and outward
            const radialDir = new THREE.Vector3(basePos.x, 0, basePos.z).normalize();
            if (radialDir.lengthSq() === 0) radialDir.set(1, 0, 0);
            sprite.position.copy(basePos).add(radialDir.multiplyScalar(2.0)).add(new THREE.Vector3(0, 2.0, 0));
            sprite.visible = false;
            
            marker.userData = { sprite: sprite, toggled: false };
            
            pigmentSpheres.push(marker);
        ...
        window.addEventListener('mouseup', (event) => {
            if (isDragging) return; // Prevent clicks from triggering after an orbit rotation
            ...
            const intersects = raycaster.intersectObjects(pigmentSpheres);
...
with open('munsell_solid_v4.html', 'w') as f:
    f.write(final_html)

print("Generated munsell_solid_v4.html successfully.")
