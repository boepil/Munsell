...
<head>
    ...
    <title>Munsell Color Solid with Pigments (v5)</title>
    <style>
        body {
            margin: 0;
            overflow: hidden;
...
<body>
    ...
    <script type="module">
        ...
        pigmentsList.forEach(pigment => {
            ...
            // Align box rotation to radius like standard voxels
            ...
            // Offset sprite slightly above the marker
            sprite.position.copy(basePos).add(new THREE.Vector3(0, 3.0, 0));
            sprite.visible = false;
            
            marker.userData = { sprite: sprite, toggled: false };
            sprite.userData = { marker: marker };
            
            pigmentBoxes.push(marker);
            pigmentSprites.push(sprite);
        ...
        window.addEventListener('mousemove', (event) => {
            ...
            if (intersects.length > 0) {
                ...
                if (hoveredPigment !== object) {
                    if (hoveredPigment && !hoveredPigment.userData.toggled) {
                        hoveredPigment.userData.sprite.visible = false;
                    }
                    hoveredPigment = object;
                    hoveredPigment.userData.sprite.visible = true;
                }
            } else {
                if (hoveredPigment) {
                    if (!hoveredPigment.userData.toggled) {
                        hoveredPigment.userData.sprite.visible = false;
                    }
                    hoveredPigment = null;
                }
        ...
        window.addEventListener('mouseup', (event) => {
            ...
            // Check sprites first (to dismiss)
            const spriteIntersects = raycaster.intersectObjects(pigmentSprites).filter(i => i.object.visible);
            if (spriteIntersects.length > 0) {
                const sprite = spriteIntersects[0].object;
                const marker = sprite.userData.marker;
                marker.userData.toggled = false;
                sprite.visible = false;
                if (hoveredPigment === marker) hoveredPigment = null;
                return;
            }
            ...
            if (intersects.length > 0) {
                ...
                marker.userData.toggled = !marker.userData.toggled;
                marker.userData.sprite.visible = marker.userData.toggled;
                // update hovered tracker if we pinned it so it stays
                if (marker.userData.toggled) hoveredPigment = marker;
            }
        });
        ...
        viewBtn.addEventListener('click', () => {
            ...
            if (isTopView) {
                ...
                camera.position.set(0, 100, 0.001); // 0.001 to prevent lookAt gimbal lock
...
with open('munsell_solid_v5.html', 'w') as f:
    f.write(final_html)

print("Generated munsell_solid_v5.html successfully.")
